...
def generate_model(opt):
    ...
    if not opt.no_cuda:
        model = model.to("cuda")
        model = nn.DataParallel(model, device_ids=list(range(_get_device_num())))
        ...
        if opt.pretrain_path:
            ...
            if opt.model in ['mobilenet', 'mobilenetv2', 'shufflenet', 'shufflenetv2']:
                ...
                model.module.classifier = model.module.classifier.to("cuda")
            elif opt.model == 'squeezenet':
                model.module.classifier = nn.Sequential(
                    nn.Dropout(p=0.5),
                ...
                model.module.fc = model.module.fc.to("cuda")
