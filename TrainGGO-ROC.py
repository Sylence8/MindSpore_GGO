...
if __name__ == '__main__':
    ...
    if isave:
        ...
        for key, value in param_dict.items():
            model_dict[key] = param_dict[key].asnumpy()

        ckpt_name = 'checkpoint_' + str(epoch) + '.ckpt'
        ckpt_path = os.path.join(save_dir, ckpt_name)
        save_checkpoint(model_dict, ckpt_path)
    ...
    if epoch >= 50 and epoch % 30 == 0:
        opt.lr = opt.lr * 0.1
        optimizer.learning_rate = opt.lr
